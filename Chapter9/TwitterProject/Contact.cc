...
void Contact::retrieveTwitterFollowing(Owner &owner) {
    ...
    if (!twitterObj.friendsIdsGet("", owner.twitterAccount)) {
    ...
    if (!twitterObj.userLookup(result, true)) {
    ...
    // string char* 변환
    char buffer[1000];
    int bufferSize = (int) result.size();
